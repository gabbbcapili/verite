...
@elseif($q->type == 'file')
    ...
    <input type="file" name="file[{{ $q->id  }}]" id="file.{{ $q->id }}" class="form-control" placeholder="{{ $q->text }}{{ $q->required ? '* ' : '' }}" @if(isset($updated_at)) title="Updated At: {{ $updated_at->diffForHumans() }}" @endif {{ isset($disabled) ? 'disabled' : '' }}>
    @if($answer)<div><a target="_blank" href="{{ asset('uploads/spaf/'. $answer)  }}">{{ $answer }}</a></div>@endif
@elseif($q->type == 'file_multiple')
    @if(!$q->next_line)<label>{{ $q->text }}{{ $q->required ? '* ' : '' }}</label>@endif
    <input type="file" multiple="multiple" name="file_multiple[{{ $q->id  }}][]" id="file_multiple.{{ $q->id }}" class="form-control" placeholder="{{ $q->text }}{{ $q->required ? '* ' : '' }}"@if(isset($updated_at)) title="Updated At: {{ $updated_at->diffForHumans() }}" @endif {{ isset($disabled) ? 'disabled' : '' }}>
    @if($answer)
                ...
                @foreach(json_decode($answer) as $index => $row)
                    <tr>
                        ...
                        @foreach($row as $column)
                            <td><input class="form-control" type="text" name="table[{{ $q->id }}][{{ $index + 1000 }}][{{ $loop->iteration }}]" value="{{ $column }}" @if(isset($updated_at)) title="Updated At: {{ $updated_at->diffForHumans() }}" @endif {{ isset($disabled) ? 'disabled' : '' }}></td>
                        @endforeach
                    </tr>
                @endforeach
            @endif
